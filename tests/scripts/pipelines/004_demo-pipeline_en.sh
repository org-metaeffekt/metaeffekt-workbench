...
set_global_variables() {
  ...
  readonly WORKSPACE_DIR="$WORKBENCH_DIR/tests/resources/workspace-004/demo"
  ...
  logger_init "$LOG_DIR/demo-pipeline_en.log"
...
convertCycloneDxToInventory() {
  CMD=(mvn -f "$KONTINUUM_PROCESSORS_DIR/convert/convert_cyclonedx-to-inventory.xml" process-resources)
  [ -n "${AE_CORE_VERSION:-}" ] && CMD+=("-Dae.core.version=$AE_CORE_VERSION")
  [ -n "${AE_ARTIFACT_ANALYSIS_VERSION:-}" ] && CMD+=("-Dae.artifact.analysis.version=$AE_ARTIFACT_ANALYSIS_VERSION")
  CMD+=("-Dinput.bom.file=$1")
  CMD+=("-Doutput.inventory.file=$2")
  CMD+=("-Dparam.reference.inventory.dir=$ENV_REFERENCE_INVENTORY_DIR")
  CMD+=("-Dparam.derive.attributes.from.purl.enabled=true")
  CMD+=("-Dparam.include.assets.enabled=true")
  CMD+=("-Dparam.include.licenses.enabled=true")

  pass_command_info_to_logger "convert_cyclonedx-to-inventory"
}
...
main() {
  ...
  convertCycloneDxToInventory \
    $WORKSPACE_DIR/stateviewer-2.0.0.0.cdx.json \
    $WORKSPACE_DIR/stateviewer-2.0.0.0-inventory_unmod.xlsx

  attachAssetMetadata \
    $WORKSPACE_DIR/stateviewer-1.7.4.0-Beta02-inventory.xlsx \
    stateviewer \
    "StateViewr" \
    "1.7.4.0-Beta02" \
    "" \
  ...
  enrichInventory \
    $WORKSPACE_DIR/stateviewer-1.7.4.0-Beta02-inventory.xlsx \
    $WORKSPACE_DIR/stateviewer-1.7.4.0-Beta02-advised-inventory.xlsx \
    $WORKSPACE_DIR/tmp \
    demo \
    default \
    "StateViewer" "1.7.4.Beta02" "Demo"

  enrichInventory \
    $WORKSPACE_DIR/stateviewer-2.0.0.0-inventory.xlsx \
    $WORKSPACE_DIR/stateviewer-2.0.0.0-advised-inventory.xlsx \
    $WORKSPACE_DIR/tmp \
    demo \
    default \
    "StateViewer" "2.0.0.0" "Demo"

  createVulnerabilityAssessmentDashboard \
    $WORKSPACE_DIR/stateviewer-2.0.0.0-advised-inventory.xlsx \
    $WORKSPACE_DIR/stateviewer-2.0.0.0-advised-inventory.html \
    demo \
    default

  createVulnerabilityAssessmentDashboard \
    $WORKSPACE_DIR/stateviewer-1.7.4.0-Beta02-advised-inventory.xlsx \
    $WORKSPACE_DIR/stateviewer-1.7.4.0-Beta02-advised-inventory.html \
    demo \
    default

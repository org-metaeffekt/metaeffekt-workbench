...
enrich_inventory() {
  ...
  DASHBOARD_SUBJECT="CryptoBOM SNAPSHOT"
...
copy_to_grouped() {
  ...
  [ -n "${AE_CORE_VERSION:-}" ] && CMD+=("-Dae.core.version=$AE_CORE_VERSION")
  [ -n "${AE_ARTIFACT_ANALYSIS_VERSION:-}" ] && CMD+=("-Dae.artifact.analysis.version=$AE_ARTIFACT_ANALYSIS_VERSION")
  CMD+=("-Dinput.inventory.dir=$WORKSPACE_DIR/crypto-bom-SNAPSHOT")
  CMD+=("-Doutput.inventory.dir=$WORKSPACE_DIR/crypto-bom-SNAPSHOT/07_grouped")
  CMD+=("-Dparam.kotlin.script.file=$WORKBENCH_DIR/scripts/group.kts")
  CMD+=("-Dparam.asset.name=crypto-bom-SNAPSHOT")
...
generate_vulnerability_assessment_dashboard() {
  ...
  OUTPUT_DASHBOARD_FILE="$ADVISED_DIR/dashboards/crypto-bom-SNAPSHOT-dashboard.html"
  SECURITY_POLICY_ACTIVE_IDS="assessment_enrichment_configuration"
...
generate_vulnerability_report() {
  ...
  OUTPUT_VR_FILE="$REPORTED_DIR/crypto-bom-vulnerability-report-de.pdf"
